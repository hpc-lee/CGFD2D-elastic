                  ...
                  const int verbose)
{
  ...
  if (bdry->is_sides_free[CONST_NDIM-1][1] == 1)
  {
    ...
    // velocity: vlow
    sv_curv_col_el_vis_rhs_vlow_z2(Vx,Vz,hTxx,hTzz,hTxz,
                                   Jxx,Jzz,Jxz,hJxx,hJzz,hJxz,
                                   xi_x, xi_z, zt_x, zt_z,
                                   lam3d, mu3d, slw3d,
                                   wl,Ylam,Ymu,
                                   vecVx2Vz,vecA,
...
//// cfs-pml, loop face inside
  if (bdry->is_enable_pml == 1)
  {
    ...
    sv_curv_col_el_vis_J(hVx,hVz,hTxx,hTzz,hTxz,
                         Jxx,Jzz,Jxz,hJxx,hJzz,hJxz,
                         lam3d, mu3d, slw3d,
                         wl,Ylam,Ymu,
                         ni1,ni2,nk1,nk2,siz_iz,nmaxwell,
                   ...
                   const int verbose)
{
  ...
  float xDVx,zDVz,zDVx,xDVz;
  float mem_Txx,mem_Tzz,mem_Txz;
  float sum_Jxx,sum_Jzz,sum_Jxz,sum_Jxxzz;
  float A[2][2], M[2][2], AM[2][2];
  ...
  for (size_t n=0; n < num_of_fdz_op-1; n++)
  {
    ...
    for (i=ni1; i<=ni2; i++)
    {
      ...
      // sum for repeated
      xDVx = xix*DxVx + ztx*DzVx;
      zDVz = xiz*DxVz + ztz*DzVz;
      xDVz = xix*DxVz + ztx*DzVz;
      zDVx = xiz*DxVx + ztz*DzVx;

      hTxx[iptr] = lam2mu * xDVx + lam * zDVz;
      hTzz[iptr] = lam2mu * zDVz + lam * xDVx;
      hTxz[iptr] = mu * (xDVz + zDVx);
                ...
                const int verbose)
{
  ...
  float EattVxx,EattVyy,EattVzz,EattVxy,EattVxz,EattVyz;
  ...
  for (size_t k=nk1; k<=nk2; k++)
  {
    ...
    for (size_t i=ni1; i<=ni2; i++)
    {
      ...
      EattVxx = ((hTxx[iptr]-hTzz[iptr])/(mu)+(hTxx[iptr]+hTzz[iptr])/(lam+mu))/4;
      EattVzz = ((hTzz[iptr]-hTxx[iptr])/(mu)+(hTxx[iptr]+hTzz[iptr])/(lam+mu))/4;
      EattVxz = hTxz[iptr]/mu*0.5;

      for (int n = 0; n < nmaxwell; n++)
      {
          hJxx[n][iptr] = wl[n]*(EattVxx-Jxx[n][iptr]);
          hJzz[n][iptr] = wl[n]*(EattVzz-Jzz[n][iptr]);
          hJxz[n][iptr] = wl[n]*(EattVxz-Jxz[n][iptr]);
      }
      ...
      hTxx[iptr] = hTxx[iptr] - mem_Txx;
      hTzz[iptr] = hTzz[iptr] - mem_Tzz;
      hTxz[iptr] = hTxz[iptr] - mem_Txz;
